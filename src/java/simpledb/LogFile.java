...
public class LogFile {
    ...
    public LogFile(File f) throws IOException {
        ...
        // log file actually corresponds to the initialpoint catalog.
        // This could cause problems since we log tableids, which may or
        // may not match tableids in the initialpoint catalog.
    }
    ...
    private void rollback(long tid) throws NoSuchElementException, IOException {
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                preAppend();
                // some code goes here
                Long recordId = tidToFirstLogRecord.get(tid);
                if (recordId == null) {
                    return;
                }
                ...
                Set<PageId> finished = new HashSet<PageId>();
                raf.seek(recordId);
                    ...
                    long curtid = raf.readLong();
                        if (type== UPDATE_RECORD)
                        {
                            if (curtid == tid) {
                                ...
                                if (!finished.contains(beforeId)) {
                                    finished.add(beforeId);
                                    DbFile dbf = Database.getCatalog().getDatabaseFile(beforeId.getTableId());
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                if (checkPointPos != -1) {
                    ...
                    raf.readLong();
                    tidToFirstLogRecord.clear();
                    int Activeaction = raf.readInt();
                    while(Activeaction> 0) {
                        long TranxId = raf.readLong();
                        long offset = raf.readLong();
                        uncommitted.add(TranxId);
                        tidToFirstLogRecord.put(TranxId, offset);
                        Activeaction--;
                    }
                    raf.readLong();
                }
                long end = raf.length();
                long initialpoint = raf.getFilePointer();
                while (initialpoint < end) {
                    int type = raf.readInt();
                    long tId = raf.readLong();
                    switch (type) {
                        case BEGIN_RECORD:
                            tidToFirstLogRecord.put(tId, initialpoint);
                            uncommitted.add(tId);
                            break;
                        case ABORT_RECORD:
                    ...
                    raf.readLong();
                    initialpoint = raf.getFilePointer();
                }
